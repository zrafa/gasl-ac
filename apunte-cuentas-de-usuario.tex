...
\texttt{cd /home/<nuevo-nombre-de-usuario>} 

\texttt{chown -R <nombre-de-usuario>.<nombre-grupo-primario> .  } 

\texttt{chmod -R go=u,go-w .}

La secuencia anterior realiza lo siguientes pasos:
\begin{enumerate}
	\item Ingresar al directorio de inicio del nuevo usuario. 
	\item Asignar la pertenencia al usuario correcto.
	\item Asignar permisos al grupo y al resto tal que: sean iguales 
	a los permisos asignados al propietario (\texttt{go=u}), a excepción del permiso 
	de escritura que se remueve (\texttt{go-w}). Si quisiéramos negar
	todos los permisos al grupo y resto de los usuarios 
	podríamos hacerlo usando ``\texttt{chmod go= .}'' en lugar de la sentencia 
	más arriba. 
\end{enumerate}
\vspace*{0.4cm} } }
